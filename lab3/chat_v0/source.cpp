...
int main(int argc, char *argv[]) {
    ...
    sb_lock_output.sem_num = 1;
    sb_lock_output.sem_op = -1;
    sb_lock_output.sem_flg = SEM_UNDO;

    sb_unlock_output.sem_num = 1;
    sb_unlock_output.sem_op = 1;
    sb_unlock_output.sem_flg = SEM_UNDO;

    if ((semkey = ftok("semdemo.c", 'L')) == -1) {
        perror("ftok");
        exit(1);
    }
